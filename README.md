...
| **중분류**      | **요구사항명**         | **기능 설명**                                        |
|-----------------|-----------------------|-----------------------------------------------------|
| 데이터 관리     | 텍스트 파일 읽기 및 분석   | 텍스트 파일에서 데이터를 읽고 청크로 나누어 처리         |
| 데이터 관리     | 데이터 임베딩 처리  | 텍스트 청크를 벡터화하여 저장                        |
| 데이터 관리     | 유사도 검색        | 입력된 키워드에 대해 유사 문서 5개 반환                |
| 데이터 초기화   | 데이터베이스 초기화   | 기존 데이터베이스를 삭제하고 새로운 데이터로 초기화      |
| 검색 기능       | 검색 로그 저장      | 검색 쿼리와 결과를 로그로 저장                       |
| 에러 처리       | 에러 핸들링    | 텍스트 파일이 비어있거나 청크 생성 실패 시 오류 메시지 제공 |
...
### 📌 6. 테스트 계획 및 결과
  ...
  ![image](img/scenario1.jpeg)
   ![image](img/scenario2.jpeg)

### 📌 7. 한 줄 회고

• 김요은: 지금까지 진행했었던 모든 프로젝트들을 진행하면서 제일 모르는 분야에 대한 두려움이 많았던 프로젝트였던 것 같습니다. 그럼에도 불구하고 잘 이끌어가고, 정리되서 나름 잘 진행되었던 프로젝트로 마무리되어 뿌듯했습니다. 또한 어떻게 해야할지 모르겠음에도 제가 이끌어가고 하자고 하는 모든 내용에 대해 군말없이 진행해준 팀원들께 감사하다고 전하고 싶습니다.  
• 박찬규:   
• 김혜서:   
• 박보람: 익숙지 않은 분야였지만, 이번 프로젝트를 통해 도전해 보고, 한층 성장할 수 있었습니다."
